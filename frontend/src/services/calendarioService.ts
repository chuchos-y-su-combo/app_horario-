...
export const calendarioService = {
    ...
    // Obtener todos los planes de estudio (para obtener IDs reales)
    getPlanesEstudio: async () => {
    const response = await axios.get(`${API_URL}/planes-estudio`);
    return response.data;
    },

    // Obtener calendario docente
    getCalendarioDocente: async (idDocente: string, semestre: string) => {
    const response = await axios.get(`${API_URL}/calendario/docente/${idDocente}?semestre=${semestre}`);
    return response.data;
    },
